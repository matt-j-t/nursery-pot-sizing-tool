...
- **Max angle from vertical: 45°.** Any surface transition steeper than this needs support and is disallowed.
...
- User selects **count: 0, 1, or 2** (if 2, positioned 180° apart).
...
- Peaked top (45°, not flat) to avoid an unsupported horizontal bridging ceiling; flat bottom.
...
- `flatTopR` = 10mm (FIXED) — the plateau's radius, sized to frame the embossed logo (~12×5mm) with comfortable
...
- `slopeRun` = 10mm (reference) — radial distance over which the rise happens.
  - Resulting angle = `atan(domeRise / slopeRun)` — **must stay ≤45°**. Reference values give 38.7°, confirmed
...
## Logo emboss
- Mark: "mjt", reference size ~12mm × 5mm, raised (not debossed), 0.5mm height.
